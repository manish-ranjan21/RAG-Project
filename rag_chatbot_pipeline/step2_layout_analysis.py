...
def extract_layout_elemnts(pdf_path: str, max_pages: int = None) -> dict:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_num, page in enumerate(page_iter, 1):
            ...
            for idx, (table_data, bbox) in enumerate(zip(tables, table_bboxes, strict=False)):
...
def group_words_into_blocks(words, exclude_bboxes):
    ...
    blocks = []
    current_block = {
        "words": [filtered_words[0]],
        "font_sizes": [filtered_words[0].get("size", 10)],
    }
    ...
    for word in filtered_words[1:]:
        ...
        same_block = abs(word["top"] - last_word["top"]) < 5 or (
