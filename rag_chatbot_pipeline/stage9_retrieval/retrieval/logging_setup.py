...
class QueryContext:
    ...
    embed_duration_seconds: Optional[float] = None
    sql_duration_seconds: Optional[float] = None
    rerank_duration_seconds: Optional[float] = None
...
@contextmanager
def log_query(query_text: str, user_id: Optional[str] = None):
    ...
    try:
        ...
        ctx.error = str(e)
        log.error(f"Query {ctx.query_id} failed: {e}",
                 extra={"extra_data": ctx.to_dict()})
        raise
    finally:
        ctx.finalize()
        if ctx.error is None:
            log.info(f"Query {ctx.query_id} ok "
