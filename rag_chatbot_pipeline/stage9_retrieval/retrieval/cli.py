...
def main():
    ...
    parser.add_argument(
        "--mock", action="store_true", help="Use mock embedder (no API key required)"
    )
    parser.add_argument("--env-file", type=Path, default=Path(".env"), help="Path to .env file")
    parser.add_argument("--health-check", action="store_true", help="Run health check and exit")
    parser.add_argument("--show-stats", action="store_true", help="Show cache stats after query")
    parser.add_argument(
        "--min-similarity", type=float, default=0.0, help="Minimum similarity threshold"
    )
    parser.add_argument(
        "--access-class", action="append", default=None, help="Access classifications (repeatable)"
    )
    parser.add_argument(
        "--access-group", action="append", default=None, help="Access groups (repeatable)"
    )
    parser.add_argument(
        "--doc-type", action="append", default=None, help="Doc types to include (repeatable)"
    )
    ...
    try:
        if args.health_check:
            ...
            health = CorpusHealthCheck(
                app.pool, app.config.embedding.model_name, app.config.embedding.dimension
            )
            result = health.run()
        ...
        if args.json:
            ...
            print(
                f"TIME:  {ctx.total_duration_seconds * 1000:.0f}ms total "
            ...
            if not results:
                ...
                print(
                    "- Permission filter excluded everything (check --access-class/--access-group)"
                )
            else:
