...
def bootstrap(env_file: Optional[Path] = None, skip_health_check: bool = False) -> Application:
    ...
    log.info(f"Embedding: {config.embedding.provider}/{config.embedding.model_name} "
    ...
    if not skip_health_check:
        health = CorpusHealthCheck(
            pool, config.embedding.model_name, config.embedding.dimension
        )
        result = health.run()
        if not result["ok"]:
        ...
        log.info(f"Health check OK: {result['chunk_count']} chunks, "
    ...
    ) if config.cache.enabled else EmbeddingCache(max_entries=1, ttl_seconds=1)
    ...
    if config.retrieval.enable_rerank:
        ...
        log.info(f"Reranker: {config.retrieval.rerank_provider} "
                 f"({config.retrieval.rerank_model}, loads on first use)")

    service = RetrievalService(pool, embedder, config.retrieval, reranker)
    
    metrics = MetricsWriter(
        config.observability.metrics_file,
        config.observability.query_log_file
    )
    
    log.info("Service ready")
    
    return Application(
        config=config, pool=pool, cache=cache,
