    ...
    def walk_outline(items, level=1, parent_path=None):
        ...
        for item in items:
            if isinstance(item, list):
                if sections:
                    sections.extend(
                        walk_outline(item, level + 1, parent_path + [sections[-1]["title"]])
                    )
            else:
                try:
...
def build_canonical_document(pdf_path: str, doc_id: str, image_routing_path: str):
    ...
    for section in sections:
        section_text = extract_section_text(pdf_path, section["page_start"], section["page_end"])
...
if __name__ == "__main__":
    ...
    )  # self-contained: docs/ & data/ live in this pipeline folder
    pdf_path = (
        base_dir
        / "docs"
    ...
    if canonical is None:
        ...
        for s in canonical["structure"][:12]:
            ...
            print(
                f"  {indent}- L{s['level']} [{s['page_range'][0]:3d}-{s['page_range'][1]:3d}] "
